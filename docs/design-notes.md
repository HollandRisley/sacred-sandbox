# Sacred Sandbox — design notes

Why the maths and the visual decisions are what they are, written down as they
were made. Several of them were wrong first; those attempts are kept here
rather than tidied away, because the reason something is not done the obvious
way is usually the only part worth reading.

[`../README.md`](../README.md) is the introduction — what the piece is and how
to run it. [`../CLAUDE.md`](../CLAUDE.md) is the short version of how not to
break it: the module map, the frame order, and the invariants.

## Running it

`npm run dev` serves at `http://localhost:5180`; the `Network:` URL Vite also
prints is the one to open on a phone on the same Wi-Fi, which is the real test.
To see whether the port is already held:
